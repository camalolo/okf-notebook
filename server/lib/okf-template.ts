/**
 * Canonical OKF v0.1 specification, embedded as a string so it survives the
 * tsc build + deploy pipeline (raw `.md` assets are not shipped — only
 * compiled JS under dist/server/).
 *
 * Seeded as `OKF.md` into every newly registered bundle that lacks one
 * (see `seedOkfSpec()` in server/bundles.ts). Keep in sync with the spec
 * copies in existing bundles.
 */

export const OKF_SPEC = `# Open Knowledge Format (OKF)
